...
Ships as an npm package, `claude-in-chrome-cli`, with `cic` as its bin, Node 22 or later, zero runtime dependencies. The plugin keeps its own entry point at `node ${CLAUDE_PLUGIN_ROOT}/bin/cic.js`, and the install test now asserts both paths produce a working `cic`: one from the installed plugin cache, one from the tarball `npm publish` would upload. `test/test_cic_cli.js` drives a scripted stub server (`test/stub_server.js`) with one misbehaviour per contract branch, asserting all five exit codes, the four `--json` error kinds, handshake ordering observed from the server side, stderr passthrough, and a reply split across chunks by a child that exits immediately after. Removes `test_request_json.sh` and `test_cic_sync.sh`, which tested the deleted script.
