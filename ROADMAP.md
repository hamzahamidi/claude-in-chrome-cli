...
## v0.8.0: user-approved adoption (shipped 2026-08-23)
...
- Establish a real drop target before taking the baseline. Call `tabs_context_mcp`; if the managed group has no tabs, `createIfEmpty: true` returns a group and its first tab together, and that tab is the **anchor**. Snapshot the managed ids, and address the anchor only by its exact id. When the group already holds a tab, that tab anchors it and nothing is created.
...
- Identify the adopted tab as one stable new live id that the bridge can actually operate: the same singleton set difference on two consecutive polls, followed by a benign read-only call against it. Detection alone is not proof, because a blank tab appearing in the group satisfies it; scheme filtering improves the prompt but capability is what settles it. Title, URL, profile and session-file metadata may guide the user but never establish identity. Zero additions before timeout or a bridge disconnect aborts without choosing a tab. Multiple additions ask the user to move the extras back out and keep waiting rather than restarting.
- Print the adopted tab as redacted live metadata and stop there. In the shell no second confirmation is asked, because adoption itself touches no page: the next line is written by a human who has just read which tab was adopted, so the confirmation is structural rather than a prompt. A form that acts immediately after adopting, such as a scripted `--then`, does not inherit that and needs explicit intent. Never close or ungroup the user's tab automatically.
- **The anchor stays open for the life of the session.** Closing the group's first tab makes the bridge lose the entire group, orphaning every other tab in it, so an anchor `cic` created is closed last and never while an adopted tab is still managed. If tabs remain at exit, leave the anchor and say why: one stray blank tab is cheaper than silently detaching the user's live tab. A cleanup failure is a warning rather than a reason to invalidate an otherwise successful adoption.
...
- Deferred, not shipped: an unattended fallback that opens the same page in a managed tab, for when the user declines or cannot intervene. It would resolve a unique passive candidate, support ordinary HTTP(S) URLs first, and keep the raw URL inside the process, never in arguments, candidate output, logs, errors or shell history. On naming, `open-copy` beat `reopen-tab` and `clone-tab`: "clone" promises DOM state, form contents, scroll position and live connections that a fresh navigation does not preserve, and this project has already shipped one claim it could not keep. The release title said "with safe reopening as fallback" and that half did not happen, so the title has been corrected rather than the memory of it.
...
- Request one upstream repair for group loss, with the evidence attached. An intact `Claude (MCP)` group is already shared across sessions, measured in both directions: a fresh session sees an earlier session's group, drives its tabs and closes them, so reuse needs nothing from `cic` beyond keeping the anchor alive. What breaks it is the first tab closing: the bridge then loses the group permanently, every session answers that no group exists while the pill sits in the tab strip with live tabs in it, and nothing can reach it again. Those unreachable corpses are what accumulate as identically named pills, closable only by hand. `cic` also cannot rename a group, since no group API crosses the bridge and page-context JavaScript cannot reach `chrome.tabGroups`; the label is the extension's.
...
Three things shipped differently from the plan above, all of them because something was measured.

The anchor rule inverted. This section says an anchor is "closed last"; it is now never closed while the group would be left empty, because emptying a group strands its pill in the tab strip and nothing can reach it again. Six create-and-close cycles left six distinct group tokens in Chrome's session file before the change, and three runs shared one group after it. The cost is accepted deliberately: one blank tab persists for good, which is cheaper than one stranded pill per run.

Both upstream requests were filed rather than merely planned, with the measurements attached, as a comment on `anthropics/claude-code#75901`, which already reported the accumulation without the mechanism.

The release also answered a question it did not set out to ask. Tab groups turned out to be extension-only surface, verified by closing every alternative: page JavaScript sees a `chrome` object holding only `loadTimes, csi, app`; CDP has no tab-group surface across its 51 domains; Chrome's AppleScript dictionary has no group vocabulary. A helper extension was built and then moved to [its own repository](https://github.com/hamzahamidi/claude-in-chrome-extension), because something asking for `tabs` should be auditable on its own terms and publishes on the Web Store's clock rather than this project's. Nothing in `cic` depends on it.

Measured against a live bridge on 2026-08-21, before any of this was built, because the whole release rests on the bridge noticing a tab the user moved rather than on Chrome being able to move it:

- **Adoption is observable on one session.** An ordinary `https` tab added through Chrome's own tab context menu appeared in `availableTabs` within seconds, held the same id across consecutive polls, and answered a `javascript_tool` read against that id.
- **Removal is observable too**, and it was checked with all three conditions the design needs: the moved tab left `availableTabs`, the anchor remained present, and the anchor was still drivable afterwards. The three recovery paths that depend on this (wrong tab moved, extras to remove, cancel after moving one in) are therefore buildable.
- **A tab id survives leaving and rejoining the group** within a session, observed once. Recorded rather than relied on: identity still comes from the live set difference every time.
- **The group belongs to its first tab.** Closing it loses the group even while another tab is open in it, and that tab is then invisible and unclosable through the bridge. This is what replaced "close the placeholder after adoption", which cannot work.
- **A conceptually invisible anchor is the right one.** A first attempt navigated the anchor and injected an instruction title so the group would be easy to find. It was easy to find, and it got closed, which ended the run. An anchor nobody is invited to touch survives; the cost is that the group is an unnamed coloured pill holding a blank tab, so the prompt should tell the user to reach it through the tab context menu rather than by spotting it.
